...
def get_most_used_keyword_for_user(username):
    """
    Επιστρέφει τη λέξη-κλειδί που εμφανίζεται στα περισσότερα
    αποθηκευμένα άρθρα ενός χρήστη.
    """
    with get_connection() as con:
        cur = con.cursor()
        cur.execute("""
...
def get_latest_comment_with_doi():
    """
    Επιστρέφει το πιο πρόσφατο σχόλιο στο σύστημα, μαζί με Username, DOI και περιεχόμενο.
    """
    with get_connection() as con:
        cur = con.cursor()
        cur.execute("""
